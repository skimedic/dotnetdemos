...
public class DataShaper<TEntity> : IDataShaper<TEntity>
{
    ...
    public DataShaper()
    {
        _properties =
            typeof(TEntity).GetProperties(BindingFlags.Public | BindingFlags.Instance)
                .ToList();
        _complexProperties =
            _properties.Where(p => p.PropertyType.IsClass && p.PropertyType != typeof(string))
                .ToList();
    }
