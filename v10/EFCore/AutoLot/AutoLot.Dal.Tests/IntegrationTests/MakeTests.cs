    ...
    IClassFixture<EnsureAutoLotDatabaseTestFixture>
{
    ...
    public void ShouldGetAllMakesAndCarsThatAreYellow()
    {
        ...
        Assert.Contains(
            ...
            x => x.Cars.Any());
        Assert.Empty(
            makes.First(m => m.Id == 1)
                .Cars);
        Assert.Empty(
            makes.First(m => m.Id == 2)
                .Cars);
        Assert.Empty(
            makes.First(m => m.Id == 3)
                .Cars);
        Assert.Single(
            makes.First(m => m.Id == 4)
                .Cars);
        Assert.Empty(
            makes.First(m => m.Id == 5)
                .Cars);
    }
