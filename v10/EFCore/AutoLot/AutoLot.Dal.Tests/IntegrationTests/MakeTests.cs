    ...
    IClassFixture<EnsureAutoLotDatabaseTestFixture>
{
    ...
    public void ShouldGetAllMakesAndCarsThatAreYellow()
    {
        ...
        Assert.Contains(makes, x => x.Cars.Any());
    ...
    {
        var query =
            Context.Makes.AsSplitQuery().IgnoreQueryFilters().Include(x => x.Cars.Where(x => x.Color == "Yellow"));
        var makes = query.ToList();
        Assert.NotNull(makes);
        Assert.NotEmpty(makes);
        Assert.Contains(makes, x => x.Cars.Any());
    ...
    [InlineData(5, 3)]
    [InlineData(6, 1)]
    public void ShouldGetAllCarsForAMakeExplicitly(
        int makeId,
        int carCount)
    {
        var make = Context.Makes.First(x => x.Id == makeId);
    ...
    [InlineData(5, 3)]
    [InlineData(6, 1)]
    public void ShouldGetAllCarsForAMakeWithFunction(
        int makeId,
        int carCount)
    {
        var query = Context.GetCarsFor(makeId).IgnoreQueryFilters();
        string qs = query.ToQueryString();
        OutputHelper.WriteLine($"Query: {qs}");
        Assert.Equal(carCount, query.Count());
    }

    [Fact]
    public void ShouldUseFunctionInServerSideQuery()
    {
        var query = Context.Makes.Where(x => ApplicationDbContext.InventoryCountFor(x.Id) > 2);
        String qs = query.ToQueryString();
